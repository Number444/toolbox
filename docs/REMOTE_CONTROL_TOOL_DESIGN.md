...
> 状态：已实现并发布（v1.5，2026-08-08） | 日期：2026-08-07 | 目标平台：Windows 10 19041+ / .NET 9 / WPF
...
> **免登录模式**（2026-08-08 决策）：无密钥启动（密钥留空 → 自动生成随机密钥）时进入免登录模式——
> 控制页不弹登录界面，任何请求直接放行（`RequireSession` 短路）。手动指定密钥启动 = 正常认证模式。
> CSRF 头校验与认证无关，两种模式均保留（跨站伪造防线不因免登录而失效）。
> 面板启动提示注明"免登录模式"；安全提示：免登录 = 局域网内任意设备可直接控制，仅建议信任网络使用。
> 已知语义（审查记录）：免登录模式下会话仅在显式调用 /api/auth 时建立 → "已连接设备"列表基本为空、
> 踢出操作无会话可踢（设备记录表仍可用）——与"无认证"语义一致，属预期；
> 面板在免登录模式下不展示/不复制密钥（避免"显示密钥但根本不校验"的误导），设备卡片显示"免登录模式无会话"。
>
> **自动填密钥缓解（2026-08-08 Kimi 审查）**：设备记录 7 天未活跃即失去自动投递密钥资格
> （DHCP 轮换后拿到旧 IP 的新设备不会自动获得密钥；重新认证即刷新恢复）。
>
> **传输层说明**：服务仅监听 IPv4（`TcpListener(IPAddress.Any)`）；
> `network` 指令为服务端预留端点，控制页当前不调用（死端点保留供扩展）；密钥/指令明文 HTTP 传输，
> 工具说明已明示"请勿在不可信网络（公共 WiFi）使用"。
...
- 2026-08-08（Kimi 独立审查修复）：免登录模式不展示/不复制密钥（消除"显示密钥但无校验"误导）；
  自动填密钥 7 天活跃期；自动启动失败原因面板可见；_authByIp 惰性清理；
  端口失焦落盘；sendCommand 5s 超时；明文 HTTP 提示入工具说明（测试 161/161）
- 2026-08-08（发布）：版本号 v1.5，安装包 55.9MB，已 push 云端
