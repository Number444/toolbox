...
- 面板 UI 规范（2026-08-11 美化）：按钮统一全局隐式 Accent 绿样式（用户决策：不分级）；
  清单/记录行项卡片化（BgCard 底 + 6px 圆角）；进度条自绘细条（BgDark 轨道 + Accent 填充，
  原生 ProgressBar 深色主题下突兀）；空状态居中图标 + 提示
- 控制页传输区（2026-08-11 美化）：上传入口为虚线投放区（移动端点按选文件，桌面浏览器可拖拽
  文件进入，`drop` → `uploadFiles`）；上传项淡入进场、文件名主色/大小次级色；
  共享清单行项卡片化，「下载」为实心 Accent 胶囊（与全局按钮语言一致）
